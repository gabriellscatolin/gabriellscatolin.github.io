...
export default class SceneInicial extends Phaser.Scene {
  ...
  adicionarBotoes() {
    this.Config.BOTOES.forEach((botao) => {
      ...
      const btn = this.add
        .image(x, y, botao.key) //Adicionar o botão à cena
  ...
  abrirCreditos() {
    ...
    add(
      this.add
        .rectangle(cx, cy, w, h, 0x000000, 0.78)
        .setDepth(depth)
        .setScrollFactor(0),
    );
    ...
    add(
      this.add
        .rectangle(cx, cy, PW, PH, 0x10141e)
        .setDepth(depth + 0.1)
        .setScrollFactor(0),
    );
    ...
    add(
      this.add
        .rectangle(cx, cy, PW, PH)
        .setStrokeStyle(2, 0x3a5ba0)
        .setDepth(depth + 0.2)
        .setScrollFactor(0),
    );
    ...
    add(
      this.add
        .rectangle(cx, cy - PH / 2 + TITLE_H / 2, PW, TITLE_H, 0x1d2b4a)
        .setDepth(depth + 0.3)
        .setScrollFactor(0),
    );
    ...
    add(
      this.add
        .text(cx, cy - PH / 2 + TITLE_H / 2, "CRÉDITOS", {
          fontSize: "28px",
          fontStyle: "bold",
          color: "#ffffff",
        })
    ...
    const imgAreaH =
      PH - TITLE_H - teamSectionH - PARTNER_SECTION_H - FOOTER_H - 12;
    const imgAreaCY = innerTop + imgAreaH / 2;

    const img = add(
      this.add
        .image(cx, imgAreaCY, "imagemCreditos")
        .setDepth(d1)
        .setScrollFactor(0),
    );
    ...
    add(
      this.add
        .rectangle(cx, sepY, PW - 40, 1, 0x2a3f6a)
        .setDepth(d1)
        .setScrollFactor(0),
    );
    ...
    const equipe = [
      {
        nome: "Sofia Farias Brandão",
        url: "https://www.linkedin.com/in/sofia-farias-brand%C3%A3o-8602b7391/",
        tipo: "li",
      },
      {
        nome: "Nicolas Dely",
        url: "https://www.linkedin.com/in/nicolas-dely-4b287b252/",
        tipo: "li",
      },
      {
        nome: "Ana Alícia Medina",
        url: "https://www.linkedin.com/in/ana-al%C3%ADcia-medina-5261431aa/",
        tipo: "li",
      },
      {
        nome: "Eduardo Melquiades",
        url: "https://www.linkedin.com/in/eduardomelquiades/",
        tipo: "li",
      },
      {
        nome: "Rachel D. Silvestre",
        url: "https://www.linkedin.com/in/racheldurantesilvestre/",
        tipo: "li",
      },
      {
        nome: "Lucas Borten",
        url: "https://www.linkedin.com/in/lucas-borten-744421215/",
        tipo: "li",
      },
      {
        nome: "Gabriel Scatolin",
        url: "https://git.inteli.edu.br/gabriel.scatolin/",
        tipo: "gl",
      },
    ];
    ...
    equipe.forEach((m, i) => {
      ...
      add(
        this.add
          .rectangle(cellLeft + 22, y, 30, 22, 0x10141e)
          .setStrokeStyle(1.5, badgeColor)
          .setDepth(d1)
          .setScrollFactor(0),
      );
    ...
    add(
      this.add
        .rectangle(cx, partnerSepY, PW - 40, 1, 0x2a3f6a)
        .setDepth(d1)
        .setScrollFactor(0),
    );
    ...
    add(
      this.add
        .text(cx, partnerSepY + 10, "PARCEIRO INSTITUCIONAL", {
          fontSize: "13px",
          fontStyle: "bold",
          color: "#5a8fd4",
        })
    ...
    const cieloCard = add(
      this.add
        .rectangle(cx, cardY, cardW, cardH, 0xffffff)
        .setStrokeStyle(1.5, 0xdddddd)
        .setDepth(d1)
        .setScrollFactor(0)
        .setInteractive({ useHandCursor: true }),
    );
    ...
    const cieloLogo = add(
      this.add
        .image(cx, cardY, "logoCielo")
        .setDepth(d1 + 0.5)
        .setScrollFactor(0),
    );

    const cieloScale = Math.min(
      (cardW - 20) / cieloLogo.width,
      (cardH - 12) / cieloLogo.height,
      1,
    );
    ...
    cieloCard.on("pointerdown", () =>
      window.open("https://www.cielo.com.br/", "_blank"),
    );
    ...
    const fchBg = add(
      this.add
        .rectangle(cx, fchY, 200, 44, 0x6a1a1a)
        .setStrokeStyle(1, 0x9a3a3a)
        .setDepth(d1)
        .setScrollFactor(0)
        .setInteractive({ useHandCursor: true }),
    );
