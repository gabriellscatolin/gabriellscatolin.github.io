export default class SceneMiniGame extends Phaser.Scene {
    ...
    create() {
        ...
        // ─── FUNDO ────────────────────────────────────────────────────
        this.fundo1 = this.add.image(GW / 2, GH / 2, 'background1')
            .setDisplaySize(GW, GH);

        // ─── PERSONAGEM ───────────────────────────────────────────────
        this.patoBranco = this.physics.add.sprite(GW * 0.2, GH * 0.5, 'patoBAndandoDireita')
            .setScale(0.3)
        ...
        // ─── PLATAFORMAS ──────────────────────────────────────────────
        this.plataformaNuvem = this.physics.add.sprite(GW * 0.28, GH * 0.62, 'plataformaNuvem');
        ...
        this.plataformaNuvem2 = this.physics.add.sprite(GW * 0.72, GH * 0.42, 'plataformaNuvem');
        ...
        this.hudBg = this.add.rectangle(20, 20, 420, 80, 0x000000, 0.45)
            .setOrigin(0, 0);
        ...
        this.timerBg = this.add.rectangle(GW - 20, 20, 320, 80, 0x000000, 0.45)
            .setOrigin(1, 0);

        // Texto do timer
        this.timerTexto = this.add.text(GW - 30, 28, this._formatarTempo(this.tempoRestante), {
            fontSize: '90px',
            fontFamily: 'poppins',
            fontStyle: 'bold',
            fill: '#ffffff',
    ...
    _tickTimer() {
        ...
        if (this.tempoFaseAtual >= 30) {
            ...
            if      (this.faseAtual === 1) this.trocarFase(2, 'background2', 'musicaFase2');
            else if (this.faseAtual === 2) this.trocarFase(3, 'background3', 'musicaFase3');
            else if (this.faseAtual === 3) this.trocarFase(4, 'background4', 'musicaFase4');
        }
    ...
    gameOver() {
        ...
        this.add.text(GW / 2, GH / 2 - 70, 'Você concluiu o Desafio!', {
            fontSize: '90px',
            fontFamily: 'poppins',
            fontStyle: 'bold',
            fill: '#0099ff',
    ...
    spawnMoedaExtra() {
        ...
        if (!this.moedaExtra.active) {
            ...
            this.time.delayedCall(6000, () => {
                if (!this.moedaExtra.active) return;
                this.tweens.add({ targets: this.moedaExtra, alpha: 0.2, duration: 200, yoyo: true, repeat: 9 });
            });
    ...
    spawnBomba() {
        ...
        if (!this.bomba.active) {
            ...
            this.time.delayedCall(4000, () => {
                if (!this.bomba.active) return;
                this.tweens.add({ targets: this.bomba, alpha: 0.2, duration: 150, yoyo: true, repeat: 9 });
            });
