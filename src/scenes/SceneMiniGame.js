class SceneMiniGame extends Phaser.Scene {
    constructor() {
        super({ key: 'SceneMiniGame' });
    }

    preload() {
        // Fundos
        this.load.image('background1', 'src/assets/imagens/imagensMiniGame/parque.png');   // Metrô fase 1
        this.load.image('background2', 'src/assets/imagens/imagensMiniGame/praia.png');    // Cielo fase 2
        this.load.image('background3', 'src/assets/imagens/imagensMiniGame/cidade.png');   // Cidade noturna fase 3
        this.load.image('background4', 'src/assets/imagens/imagensMiniGame/noite.png');    // Espaço cideral Cielo fase 4
     
        // Itens coletáveis
        this.load.image('moedaComum', 'src/assets/imagens/imagensMiniGame/moedaComum.png');        
        this.load.image('moedaExtra', 'src/assets/imagens/imagensMiniGame/moedaExtra.png');     
        this.load.image('bomba', 'src/assets/imagens/imagensMiniGame/bomba.png');  

        // Plataforma de nuvem
        this.load.image('plataformaNuvem', 'src/assets/imagens/imagensMiniGame/plataformaNuvem.png'); 

        // Sprites do personagem 
        this.load.spritesheet('patoBAndandoDireita', 'src/assets/imagens/imagensMiniGame/sprite/patoBAnadandoDireita.png', { frameWidth: 1024, frameHeight: 1024 });   
        this.load.spritesheet('patoBAndandoEsquerda', 'src/assets/imagens/imagensMiniGame/sprite/patoBAndandoEsquerda.png', { frameWidth: 1024, frameHeight: 1024 });  
        this.load.spritesheet('personagemPulando', 'src/assets/imagens/imagensMiniGame/sprite/personagemPulando.png', { frameWidth: 1024, frameHeight: 1024 });   
        this.load.spritesheet('personagemParado', 'src/assets/imagens/imagensMiniGame/sprite/personagemParado.png', { frameWidth: 1024, frameHeight: 1024 }); 

        // Carregar músicas e toque sonoro de coleta de itens
        this.load.audio('musicaFase1', 'src/assets/imagens/imagensMiniGame/musicas/musicaCenaParque.mp3');  // Fase 1
        this.load.audio('musicaFase2', 'src/assets/imagens/imagensMiniGame/musicas/musicaCenaPraia.mp3');   // Fase 3
        this.load.audio('musicaFase3', 'src/assets/imagens/imagensMiniGame/musicas/musicaCenaCidade.mp3');  // Fase 4
        this.load.audio('musicaFase4', 'src/assets/imagens/imagensMiniGame/musicas/musicaCenaNoite.mp3');   // Fase 5
        this.load.audio('somColeta', 'src/assets/imagens/imagensMiniGame/musicas/somColetaitem.mp3');       // Áudio coleta de item

        }

    create() {
        // Estado inicial
        this.faseAtual = 1;
        this.pontuacao = 0;           // "Variável" da pontuação do jogador 
        this.direcao = 'direita';     // "Variável" da direção direita 

        // Criar sons 
        this.musica = this.sound.add('musicaFase1', { loop: true, volume: 0.5 });
        this.musica.play();

        this.somColeta = this.sound.add('somColeta', { volume: 0.7 });  // Criar som de coleta 

        // Adcionando fundo fase 1
        this.fundo1 = this.add.image(1500/2, 940/2, 'background1');  

        // Adicionando física no player
        this.patoBranco = this.physics.add.sprite(300, 500, 'patoBAndandoDireita')
        .setScale(0.1)
        .setCollideWorldBounds(true);

        // Adicionando movimentações do teclado
        this.teclado = this.input.keyboard.createCursorKeys(); 

        //Criar chão colidível
        this.chao1 = this.physics.add.staticImage(1500/2, 1140 - 20, null);  

        // Ajustar tamanho e aparencia da colisão do chão 
        this.chao1.displayWidth = 1500;         
        this.chao1.displayHeight = 470;         
        this.chao1.refreshBody();             
        this.chao1.setVisible(false);          

        // Colisão pesonagem e chao 
        this.physics.add.collider(this.patoBranco, this.chao1);

        /* Criar plataformas de nuvem */
        // Primeira plataforma:
        this.plataformaNuvem = this.physics.add.sprite(500, 350, 'plataformaNuvem');

        this.plataformaNuvem.setScale(0.5);

        this.plataformaNuvem.body.setImmovable(true);
        this.plataformaNuvem.body.allowGravity = false;
        this.plataformaNuvem.body.setSize(650, 80);   
        this.plataformaNuvem.body.setOffset(450, 500); 

        this.physics.add.collider(this.patoBranco, this.plataformaNuvem); // Colisão entre o pato e nuvem

        // Segunda plataforma:
        this.plataformaNuvem2 = this.physics.add.sprite(1000, 600, 'plataformaNuvem');

        this.plataformaNuvem2.setScale(0.5);

        this.plataformaNuvem2.body.setImmovable(true);
        this.plataformaNuvem2.body.allowGravity = false;
        this.plataformaNuvem2.body.setSize(650, 80);
        this.plataformaNuvem2.body.setOffset(450, 500);

        this.physics.add.collider(this.patoBranco, this.plataformaNuvem2);  // Colisão da segunda plataforma de nuvem com o pato

        /*ADICIONANDO MOEDAS*/
        // Moeda comum
        this.moedaComum = this.physics.add.sprite(1500/2, 0, 'moedaComum');
        this.moedaComum.setScale(0.09);                                                   
        this.moedaComum.body.setSize(this.moedaComum.width * 0.6, this.moedaComum.height * 0.6);  
        this.moedaComum.setCollideWorldBounds(true);                                      
        this.moedaComum.setBounce(0.7);                                                   

        this.physics.add.collider(this.moedaComum, this.plataformaNuvem);                 
        this.physics.add.collider(this.moedaComum, this.plataformaNuvem2);               
        this.physics.add.collider(this.moedaComum, this.chao1);                           

        // Moeda extra 
        this.moedaExtra = this.physics.add.sprite(0,0, 'moedaExtra');
        this.moedaExtra.setScale(0.09);
        this.moedaExtra.body.setSize( this.moedaExtra.width * 0.6, this.moedaExtra.height * 0.6);
        this.moedaExtra.setCollideWorldBounds(true);
        this.moedaExtra.setBounce(0.7);

        this.moedaExtra.setVisible(false);    // Começa invísivel
        this.moedaExtra.setActive(false);     // Desativa a física

        this.physics.add.collider(this.moedaExtra, this.plataformaNuvem);          // Colisão da moeda extra com a primeira plataforma de nuvens
        this.physics.add.collider(this.moedaExtra, this.plataformaNuvem2);         // Colisão da moeda extra com a segunda plataforma de nuvens 
        this.physics.add.collider(this.moedaExtra, this.chao1);                    // Colisão da moeda extra com o chão

        // Adicionando bomba 
        this.bomba = this.physics.add.sprite(0, 0, 'bomba');
        this.bomba.setScale(0.11);
        this.bomba.body.setSize(this.bomba.width * 0.6, this.bomba.height * 0.6);
        this.bomba.setCollideWorldBounds(true);
        this.bomba.setBounce(0.7);
        this.bomba.disableBody(true, true);       

        this.physics.add.collider(this.bomba, this.plataformaNuvem);          // Colisão da bomba com a primeira plataforma de nuvens
        this.physics.add.collider(this.bomba, this.plataformaNuvem2);         // Colisão da bomba com a segunda plataforma de nuvens 
        this.physics.add.collider(this.bomba, this.chao1);                    // Colisão da bomba com o chão

        /*Adicionando o placar*/
        this.placar = this.add.text(50,50, 'Cielocoins: ' + this.pontuacao, {fontSize: '45px', fontFamily: 'poppis', fontStyle: 'bold', fill:'#ffffff'}); 

        /*Eventos de tempo*/
        // Fazer as moedas e a bomba cair periodicamente
        // Evento que cria as moedas extras a cada 20 segundos
        this.time.addEvent({
            delay: 20000,   
            callback: this.spawnMoedaExtra,
            callbackScope: this,
            loop: true
        });

        //Tempo da bomba
        this.time.addEvent({
            delay: 10000,
            loop: true,
            callback: () => {
                this.spawnBomba();
        }
        });

        // Detectar que a moeda extra vale 3 pontos 
        this.physics.add.overlap(this.patoBranco, this.moedaExtra, () => {

        if (!this.moedaExtra.active) return;

        this.somColeta.play();

        this.moedaExtra.disableBody(true, true);

        this.pontuacao += 3;
        this.placar.setText('Cielocoins: ' + this.pontuacao);
        });
        
        
        // Detectar que a bomba vale -1 ponto
        this.physics.add.overlap(this.patoBranco, this.bomba, () => {

        if (!this.bomba.active) return;

        this.somColeta.play();

        this.bomba.disableBody(true, true);

        this.pontuacao -= 1;

        if (this.pontuacao < 0) {
            this.pontuacao = 0; // evita pontuação negativa
        }

        this.placar.setText('Cielocoins: ' + this.pontuacao);
        });


        // Quando o personagem encostar na moeda comum...
        this.physics.add.overlap(this.patoBranco, this.moedaComum, () => {
            this.moedaComum.setVisible(false);  // Moeda fica invisível

            this.somColeta.play();                                      // Som de coleta da moeda
            let posicaoMoeda_Y = Phaser.Math.RND.between(10,1500);      // Sorteia número (gera nova posição aleatória para a moeda)
            this.moedaComum.setPosition(posicaoMoeda_Y, 50);            // Move a moeda para o topo
            this.pontuacao +=1;                                         // Soma pontuação
            this.placar.setText('Cielocoins:'+ this.pontuacao,);        // Atualiza texto no placar
            this.moedaComum.setVisible(true);                           // Ativa a visão da "nova moeda"

        });
        
        /*ANIMAÇÕES*/
        // Animação andando para direita
        this.anims.create({
            key: 'andar_direita',  
            frames: this.anims.generateFrameNumbers('patoBAndandoDireita', { start: 0, end: 2 }),
            frameRate: 10,
            repeat: -1
        });

        // Animação andando esquerda
        this.anims.create({
            key: 'andar_esquerda', 
            frames: this.anims.generateFrameNumbers('patoBAndandoEsquerda', { start: 0, end: 2 }),
            frameRate: 10,
            repeat: -1
        });

        // Pulo do personagem 
        this.anims.create({
            key: 'pulo',
            frames: this.anims.generateFrameNumbers('personagemPulando', { start: 0, end: 0 }),
            frameRate: 10,
            repeat: -1
        });

        // Personagem parado
        this.anims.create({
            key: 'parado',
            frames: this.anims.generateFrameNumbers('personagemParado', { start: 0, end: 0 }),
            frameRate: 10,
            repeat: -1
        });
        
    }
 
    update() {
        // Movimento horizontal (eixo x)
        this.patoBranco.setVelocityX(0);

        // Movimento para esquerda [ <- ]
        if (this.teclado.left.isDown) {
            this.patoBranco.setVelocityX(-300); 
            this.direcao = 'esquerda';
        }

        // Movimento para direita [ -> ]
        else if (this.teclado.right.isDown) {
            this.patoBranco.setVelocityX(300); 
            this.direcao = 'direita';
        }

        // Pulo do personagem (eixo y)
        if (this.teclado.up.isDown && this.patoBranco.body.touching.down) {
            this.patoBranco.setVelocityY(-570);
        }

        // Personagem no ar (Pulando)
        if (this.patoBranco.body.velocity.y !== 0) {

            this.patoBranco.anims.play('pulo', true);

        } else {

            //  Personagem andando
            if (this.patoBranco.body.velocity.x > 0) {
                this.patoBranco.anims.play('andar_direita', true);
            }
            else if (this.patoBranco.body.velocity.x < 0) {
                this.patoBranco.anims.play('andar_esquerda', true);
            }

            // Personagem parado
            else {
                this.patoBranco.anims.play('parado', true);
            }
        }

        /*TROCAS DE FASES*/
        if (this.pontuacao >= 10 && this.faseAtual === 1) {
            this.trocarFase(2, 'background2', 'musicaFase2');
        }

        if (this.pontuacao >= 20 && this.faseAtual === 2) {
            this.trocarFase(3, 'background3', 'musicaFase3');
        }

        if (this.pontuacao >= 30 && this.faseAtual === 3) {
            this.trocarFase(4, 'background4', 'musicaFase4');
        }

        if (this.pontuacao >= 40) {
            this.gameOver();
        }
    }

    /*FUNÇÕES EXTRAS DO JOGO*/
    trocarFase(novaFase, novaImagem, novaMusicaKey) {
        this.faseAtual = novaFase;

        // Fade 
        this.cameras.main.fade(400, 0, 0, 0);

        this.time.delayedCall(400, () => {

            // Trocar fundo
            this.fundo1.setTexture(novaImagem);

            // Parar música atual
            if (this.musica) {
                this.musica.stop();
            }

            // Criar nova música
            this.musica = this.sound.add(novaMusicaKey, {
                loop: true,
                volume: 0.5
            });

            this.musica.play();

            this.cameras.main.fadeIn(400, 0, 0, 0);
        });
    }

    /*TELA GAME OVER (quando atingir 120 moedas coletadas)*/
    gameOver() {

        this.musica.stop();
        this.physics.pause();

        this.add.rectangle(750, 470, 1500, 940, 0x000000, 0.6);

        this.add.text(750, 470, 'GAME OVER', {
            fontSize: '100px',
            fontFamily: 'poppis',
            fill: '#0099ff'
        }).setOrigin(0.5);

    }

    spawnMoedaExtra() {
        if (!this.moedaExtra.active) {
            let posicaoX = Phaser.Math.Between(100, 1400);

            // Ativar corpo físico corretamente
            this.moedaExtra.enableBody(true, posicaoX, 0, true, true);

            this.moedaExtra.setAlpha(1);

            this.time.delayedCall(6000, () => {             // Depois de 6 segundos começa a piscar

                if (!this.moedaExtra.active) return;

                this.tweens.add({
                    targets: this.moedaExtra,
                    alpha: 0.2,          
                    duration: 200,
                    yoyo: true,
                    repeat: 9            
                });

            });

            this.time.delayedCall(8000, () => {              // item desaparece completamente depois de 8 segundos


                if (this.moedaExtra.active) {
                    this.moedaExtra.disableBody(true, true);
                }

            });
        }
    }

    spawnBomba() {

        if (!this.bomba.active) {
            let posicaoX = Phaser.Math.Between(100, 1400);

            this.bomba.enableBody(true, posicaoX, 0, true, true);
            this.bomba.setAlpha(1);

            this.time.delayedCall(4000, () => {             // Começa a piscar depois de 4 segundos


                if (!this.bomba.active) return;

                this.tweens.add({
                    targets: this.bomba,
                    alpha: 0.2,
                    duration: 150,
                    yoyo: true,
                    repeat: 9
                });

            });

            this.time.delayedCall(6000, () => {              // Desaparece depois de 6 segundos

                if (this.bomba.active) {
                    this.bomba.disableBody(true, true);
                }

            });
        }
    }
}

// Tornar a classe acessível globalmente
window.sceneFase1 = sceneFase1;

